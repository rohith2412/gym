...
export async function checkAiLimit(userId, feature) {
  ...
  // Ensure the mongoose connection is up before we touch the collection.
  // Serverless cold starts otherwise leave the driver in "buffering" mode
  // and `findOne` times out after 10s with the generic "aiusages.findOne()
  // buffering timed out" error — that's what was breaking photo and voice.
  await connectdb();
