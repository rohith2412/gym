...
/**
 * Generate a 6-char referral code from an unambiguous alphabet — no 0/O,
 * no 1/I/L. Users read these off a share sheet and type them manually, so
 * every character has to survive a squint.
 */
const ALPHABET = "ABCDEFGHJKMNPQRSTUVWXYZ23456789"; // 31 chars

export function makeRandomCode(len = 6) {
  let out = "";
  for (let i = 0; i < len; i++) {
...
export async function ensureCode(userId) {
  const existing = await Auth.findById(userId).select("code").lean();
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    ...
    try {
      ...
      const now = await Auth.findById(userId).select("code").lean();
